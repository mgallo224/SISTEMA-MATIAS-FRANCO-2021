...
def cventas():
    ...
    def pdf(obtener_producto,obtener_cantidad,obtener_precio,fecha,obmes,obaño,nombre):
        rand = random.randint(1,999)
        cvas = canvas.Canvas("facturas/"+str(nombre)+"_"+str(obmes)+"-"+str(obaño)+"___"+str(rand)+".pdf", pagesize=letter)
        cvas.setLineWidth(.3)
        ...
        cvas.save() 
        return messagebox.showinfo("Carga exitosa", "El pedido fue cargado exitosamente, se genero en la carpeta del programa la factura de nombre: "+str(nombre)+"_"+str(obmes)+"-"+str(obaño)+"___"+str(rand)+".pdf")
    ...
    def añadir_a_la_base():
        ...
        if not obtener_producto0:
        ...
        elif len(obtener_nombreyapellidocliente)<=0 or obtener_nombreyapellidocliente.isnumeric() == True:
            return messagebox.showerror('Error', 'El campo "Nombre" es invalido')
            ...
            if parallevar == 1 and (len(obtener_direccioncliente)<=0):
                ...
                for obtener_producto0 in obtener_producto0:
                    ...
                    if data0[0]<int(obtener_cantidad):
                        ...
                        b = datap[2]
                        error = a + b
                        return messagebox.showerror("Error al cargar pedido", error)
                    else:    
                        ...
                        conn.commit()
                        lista_productos.append(data0[2])
                        print(lista_productos)
                        pdfproducto = "/".join(lista_productos)
                pdf(pdfproducto,obtener_cantidad,obtener_precio,fecha,obmes,obaño,obtener_nombreyapellidocliente)
            cursor.close()    
    ...
    if __name__ == "modulos.cventas":
        ...
        nombreyapellidocliente_label = Label(ventana_cventas, text="A nombre de:")
        nombreyapellidocliente_label.place(x=20, y=200)
        nombreyapellidocliente = tk.Entry(ventana_cventas)
        # Posicionarla en la ventana.
        nombreyapellidocliente.place(x=200, y=200)
